...
export function useAgentStream(callbacks: AgentStreamCallbacks, threadId: string, setMessages: (messages: UnifiedMessage[]) => void): UseAgentStreamResult {
  ...
  const handleStreamError = useCallback((err: Error | string | Event) => {
    ...
    // Check if this is a "not found" error which is expected when agent has completed
    if (errorMessage.includes('not found') || errorMessage.includes('404') || errorMessage.includes('does not exist')) {
      console.warn('[useAgentStream] Streaming error (expected if agent completed):', errorMessage);
    } else {
    ...
    getAgentStatus(runId)
      ...
      .catch(statusError => {
        ...
        if (isNotFoundError) {
           console.warn(`[useAgentStream] Agent run ${runId} not found after stream error (expected if completed): ${statusErrorMessage}`);
     ...
     getAgentStatus(runId)
      .then(agentStatus => {
       ...
       .catch(err => {
          ...
          if (errorMessage.includes('not found') || errorMessage.includes('404') || errorMessage.includes('does not exist')) {
            console.warn(`[useAgentStream] Agent run ${runId} not found after stream close (expected if completed): ${errorMessage}`);
          } else {
