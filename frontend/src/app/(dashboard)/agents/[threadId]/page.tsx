...
export default function ThreadPage({ params }: { params: Promise<ThreadParams> }) {
  ...
  useEffect(() => {
    if (projectName) {
      ...
      document.title = `${projectName} | Orchestra Connect`;

      // Update meta tags for SEO
      const metaDescription = document.querySelector('meta[name="description"]');
      if (metaDescription) {
        metaDescription.setAttribute('content', `${projectName} - Interactive agent conversation powered by Orchestra Connect`);
      }

      // Update OpenGraph tags if they exist
      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | Orchestra Connect`);
      }
