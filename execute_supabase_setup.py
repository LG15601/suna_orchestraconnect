...
# Lire le script SQL
SCRIPT_PATH = 'supabase_setup.sql'

# Créer le fichier SQL avec le contenu fourni
with open(SCRIPT_PATH, 'w') as f:
    f.write('''-- 1. Basejump setup
/**
      ____                 _
     |  _ \               (_)
...
print(f"Script SQL créé : {SCRIPT_PATH}")

# Exécuter le script SQL dans Supabase
SUPABASE_REST_URL = f"{SUPABASE_URL}/rest/v1/rpc/exec_sql"
...
print("Création de la fonction exec_sql dans Supabase si elle n'existe pas...")
create_function_sql = """
...
try:
    create_function_response = requests.post(
        f"{SUPABASE_URL}/rest/v1/sql",
        headers=headers,
        json={"query": create_function_sql}
    )
    print(f"Création de la fonction exec_sql: {create_function_response.status_code} {create_function_response.text}")
    if not create_function_response.ok and "already exists" not in create_function_response.text.lower():
        print(f"Erreur lors de la création de la fonction exec_sql: {create_function_response.text}")
        exit(1)
    else:
        print("Fonction exec_sql créée ou déjà existante")
except Exception as e:
    print(f"Erreur lors de la création de la fonction exec_sql: {str(e)}")
    exit(1)

# Exécuter le script SQL par morceaux pour éviter les problèmes de taille
CHUNK_SIZE = 8000  # ~8KB par morceau
sql_chunks = [sql_script[i:i+CHUNK_SIZE] for i in range(0, len(sql_script), CHUNK_SIZE)]

print(f"Exécution du script SQL en {len(sql_chunks)} morceaux...")

for i, chunk in enumerate(sql_chunks):
    print(f"Exécution du morceau {i+1}/{len(sql_chunks)}...")
    try:
        # Appeler la fonction exec_sql
        response = requests.post(
            SUPABASE_REST_URL,
            headers=headers,
            json={"sql_query": chunk}
        )
        
        if response.status_code >= 400:
            print(f"Erreur lors de l'exécution du morceau {i+1}: {response.status_code} {response.text}")
        else:
            print(f"Morceau {i+1} exécuté avec succès.")
    except Exception as e:
        print(f"Erreur lors de l'exécution du morceau {i+1}: {str(e)}")

print("\nScript SQL exécuté avec succès!")
print("Vérifiez la base de données pour vous assurer que toutes les tables ont été créées correctement.")